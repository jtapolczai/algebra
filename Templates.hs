...
module Templates (makeContentTag, makeEnumTag, makeStructureTypeSynonym) where
...
{-| Creates a type synonym for a specific algebraic structure.
    ...
    @
      type name v1 ... vq = struct f(t1) ... f(tn) v1 ... vq
        where f(ti) = if ti element of P then "Tag" ++ ti
                      else "TagUnknown" ++ ti
    @

    An example would be the creation of a commutative and associative structure,
    where the underlying data type also supports idempotence:

    @
      makeStructureTypeSynonym "CommAss" "GroupStruct" ["Associative", "Commutative"]
                               ["Associative", "Commutative", "Idempotent"] ["elemType"]

      type ComAss elemType = GroupStruct TagAssociative TagCommutative TagUnknownIdempotent elemType
    @
-}
makeStructureTypeSynonym :: String -> String -> [String] -> [String] -> [String] -> DecsQ
makeStructureTypeSynonym name struct present traits vars = sequenceQ [tyName tyBody]
  where cxt' = returnQ []
        ...
        mkTrait t | t `elem` present = conT $ mkName $ "Tag" ++ t
                  | otherwise        = conT $ mkName $ "TagUnknown" ++ t 
