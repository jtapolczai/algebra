...
addGroup :: Num a => CommutativeGroupStruct a String
addGroup = makeCommutativeGroup (+) ((*) $ negate $ fromInteger 1) (fromInteger 0) "add"

-- |The Abelian group (Fractional, *, 0, \x -> 1/x) for types that support division
mulGroup :: Fractional a => CommutativeGroupStruct a String
mulGroup = makeCommutativeGroup (+) ((fromInteger 1)/) (fromInteger 0) "mul"

-- |The abelian group (Complex Fractional, \(a+bi) (c+di) -> (a+c) + (b+d)i, \(a+bi) -> (-a - bi), (0+0i))
--  for complex numbers.
comAddGroup :: RealFloat a => CommutativeGroupStruct (Complex a) String
comAddGroup = makeCommutativeGroup (\x y -> (realPart x + realPart y) :+ (imagPart x + imagPart y))
                                   (\x -> (negate $ realPart x) :+ (negate $ imagPart x))
                                   (fromInteger 0 :+ fromInteger 0) "comAdd"

-- |The abelian group (Complex Fractional, \(a+bi) (c+di) -> (ac - bd) + (ad + bc)i, \(a+bi) -> (1/a + a/b²) + (b/a² - 1/b)i, (1+0i))
--  for complex numbers.
comMulGroup :: RealFloat a => CommutativeGroupStruct (Complex a) String
